...
namespace demonorium
{
	namespace utils
	{
		...
		public:
			explicit SimpleTree(const Alloc& alloc = Alloc());
			~SimpleTree();
		...
		inline TreeNode<T>* SimpleTree<T, Alloc>::balance(TreeNode<T>* p) {
			...
			if (bfactor(p) == 2) {
				if (bfactor(p->right) < 0)
					p->linkRight(rotateRight(p->unlinkRight()));
				return rotateLeft(p);
			}
			if (bfactor(p) == -2) {
				if (bfactor(p->left) > 0)
					p->linkLeft(rotateLeft(p->unlinkLeft()));
		...
		TreeNode<T>* SimpleTree<T, Alloc>::insert(TreeNode<T>* root, TreeNode<T>* node) {
			...
			if (node->content < root->content)
				root->linkLeft(insert(root->unlinkLeft(), node));
			else
				root->linkRight(insert(root->unlinkRight(), node));
